export const obras = [
    {
        'id': 'Paikam',
        'name': "Paik'am", 
        'intro':'En una tierra sin pasos, sin huellas como el agua, hubo un tiempo en que habitó una raza de guerreros, hombres y mujeres valientes, hijos de los dioses...', 
        'more':'“Que todo el mundo conozca nuestra historia, antes que el polvo entierre nuestro origen”',
        'credits': [
            {
                'id': 1,
                'rol': 'Director',
                'name': 'Alfredo Payán'
            },
            {
                'id': 2,
                'rol': 'Producción',
                'name': 'Sara Don y Alfredo Payán'
            },
            {
                'id': 3,
                'rol': 'Asistente de dirección',
                'name': 'Noemí Mendiola Egure'
            },
            {
                'id': 14,
                'rol': 'Asistente de Audio',
                'name': 'Luis Fernando Vallejo García'
            },
            {
                'id': 15,
                'rol': 'Asistente de iluminación',
                'name': 'Daniel Diaz Téllez'
            },
            {
                'id': 0,
                'rol': 'Narraturgia',
                'name': 'Gilberto Lastra y Alfredo Payán'
            },
            {
                'id': 0,
                'rol': 'Elenco',
                'elenco' : [
                    {
                        'id': 4,
                        'name': 'Melissa Zaragoza'
                    },
                    {
                        'id': 5,
                        'name': 'Karen Covarrubias'
                    },
                    {
                        'id': 6,
                        'name': 'Brian Smithe'
                    },
                    {
                        'id': 7,
                        'name': 'Héctor Morán'
                    },
                    {
                        'id': 8,
                        'name': 'Francisco Ávila Casas'
                    },
                    {
                        'id': 9,
                        'name': 'Erik Martínez'
                    },
                    {
                        'id': 10,
                        'name': 'Héctor Manuel Arredondo'
                    },
                ]
            },
            {
                'id': 11,
                'rol': 'Narrador',
                'name': 'Mario Montaño'
            },
            {
                'id': 12,
                'rol': 'Música Original',
                'name': 'Roberto García'
            },
            {
                'id': 0,
                'rol': '“El niño que saltó la luna”',
                'name': 'Letra: Héctor Manuel Arredondo • Música: Roberto García'
            },
            {
                'id': 0,
                'rol': '"Que chula estrella”',
                'name': 'Canción del dominio popular • Interpretan: “Mujeres Cardencheras” Evangelina Nuñez, Catalina Bañuelos, Guadalupe Ríos y Alma Montenegro'
            },
            {
                'id': 12,
                'rol': 'Arreglos Musicales',
                'name': 'Roberto García'
            },
            {
                'id': 0,
                'rol': 'Diseño de imagen',
                'name': 'Pepo Almonte'
            },
            {
                'id': 13,
                'rol': 'Diseño de títeres',
                'name': 'Edwin Salas'
            }

        ],
        'mainImg':'/images/paikam_1.jpg',
        'imgPos': 'down',
        'video': 'https://www.youtube.com/embed/OfjEo-lIOdk',
        'gallery': [
            {
                'id': 'paikam01',
                'url': '/images/paikam01.jpg',
                'description': 'Inspirado en el origen mitológico de Durango desde la cosmogonía Tepehuana, Maika Teatro presenta: “Pai´kam”, un espectáculo de títeres, danza, sombras y objetos para jóvenes y adultos, basado en “La leyenda de Sahuatoba” y “La montaña de plata” historias que hablan del surgimiento de la raza humana y del nacimiento del cerro de Mercado, que dio pié a la fundación de Durango y de la primera fundidora de hierro, no sólo del país, sino de toda América Latina.'
            },
            {
                'id': 'paikam02',
                'url': '/images/paikam02.jpg',
                'description': 'Pai´kam surge como resultado del Programa de formación para nuevos titiriteros en Durango.'
            },
            {
                'id': 'paikam03',
                'url': '/images/paikam03.jpg',
                'description': '<h5>Proyecto apoyado por el Programa Creadores Escénicos, FONCA 2019.</h5>'
            },
            {
                'id': 'paikam_thumb_01',
                'url': '/images/paikam_thumb_01.jpg'
            },
            {
                'id': 'paikam_thumb_02',
                'url': '/images/paikam_thumb_02.jpg'
            },
            {
                'id': 'paikam_thumb_03',
                'url': '/images/paikam_thumb_03.jpg'
            },
            {
                'id': 'paikam_thumb_04',
                'url': '/images/paikam_thumb_04.jpg'
            },
            {
                'id': 'paikam_thumb_05',
                'url': '/images/paikam_thumb_05.jpg'
            },
            {
                'id': 'paikam_thumb_06',
                'url': '/images/paikam_thumb_06.jpg'
            }
        ]
    },
    {
        'id': 'Ofelia',
        'name': "Ofelia", 
        'intro':'Cuando Ofelia nació, sus padres deseaban que su hija fuera una gran actriz y ella heredó su amor por el teatro. Pero como tenía una voz muy bajita, encontró otra forma de vivir cerca de aquello que la apasionaba: desde el puesto del apuntador...', 
        'more':'',
        'credits': [
            {
                'rol': 'Dirección y Adaptación',
                'name': 'Alfredo Payán'
            },
            {
                'rol': 'Animación',
                'name': 'Montse Gaxiola, Víctor Villa y Leonardo García'
            },
            {
                'rol': 'Asistente',
                'name': 'Daniel García'
            },
            {
                'rol': 'Elaboración de Títeres',
                'name': 'Sara Don, Iván Romo y Alfredo Payán'
            },
            {
                'rol': 'Música Original',
                'name': 'Manu García Campos'
            },
            {
                'rol': 'Escenografía',
                'name': 'Alfredo Payán'
            }
        ],
        'mainImg':'/images/ofelia_1.jpg',
        'imgPos': 'up',
        'gallery': [
            {
                'id': 'ofelia_01',
                'url': '/images/ofelia_01.jpg',
                'description': 'Cuando el teatro cerró, ni ella misma imaginaría la aventura que le esperaba, acompañada de seres misteriosos salidos de los rincones más profundos del teatro: las sombras.'
            },
            {
                'id': 'ofelia_02',
                'url': '/images/ofelia_02.jpg',
                'description': 'Maika Teatro recrea a través del universo surrealista de los títeres y el teatro de sombras una delicada y fantástica historia inspirada en el cuento de Michael Ende: El teatro de sombras de Ofelia.',
            },
            {
                'id': 'ofelia_03',
                'url': '/images/ofelia_03.jpg',
                'description': ''
            },
            {
                'id': 'ofelia_thumb_01',
                'url': '/images/ofelia_thumb_01.jpg',
            },
            {
                'id': 'ofelia_thumb_02',
                'url': '/images/ofelia_thumb_02.jpg',
            },
            {
                'id': 'ofelia_thumb_03',
                'url': '/images/ofelia_thumb_03.jpg',
            },
            {
                'id': 'ofelia_thumb_04',
                'url': '/images/ofelia_thumb_04.jpg',
            },
            {
                'id': 'ofelia_thumb_05',
                'url': '/images/ofelia_thumb_05.jpg',
            },
            {
                'id': 'ofelia_thumb_06',
                'url': '/images/ofelia_thumb_06.jpg',
            }
        ]
    },
    {
        'id': 'Pintor',
        'name': "El Pintor de Mariposas", 
        'intro':'Al pueblo ha llegado Juan, un anciano autoexiliado de la sociedad, inmerso en su propia soledad y en su gran pasión: la pintura...',
        'more': 'Es ahí donde encuentra la capacidad de generar vínculos sólidos con seres pequeños y poco comunes como las mariposas, observando en ellas detalles únicos y especiales que lo llevan a descubrir un mundo donde puede sentirse identificado y realizado.',
        'credits': [
            {
                'rol': 'Dirección y Adaptación',
                'name': 'Alfredo Payán'
            },
            {
                'rol': 'Animación',
                'name': 'Manu García Campos y Alfredo Payán'
            },
            {
                'rol': 'Asistente de Dirección',
                'name': 'Manu García Campos'
            },
            {
                'rol': 'Producción',
                'name': 'Sara Don Luna'
            },
            {
                'rol': 'Elaboración de Títeres',
                'name': 'Sara Don, Iván Romo y Alfredo Payán'
            },
            {
                'rol': 'Música original',
                'name': 'Manu García Campos'
            },
            {
                'rol': 'Diseño de Vestuario',
                'name': 'Sara Don'
            },
            {
                'rol': 'Escenografía',
                'name': 'Alfredo Payán'
            },
            {
                'rol': 'Diseño de Iluminación',
                'name': 'Manu García Campos'
            },
            {
                'rol': 'Asesoría en Pantomima',
                'name': 'Alberto Stanley'
            },
            {
                'rol': 'Voz en off',
                'name': 'Diego Alonso Payán'
            }
        ],
        'mainImg':'/images/pintor_1.jpg',
        'imgPos': 'down',
        'video': 'https://www.youtube.com/embed/a6RHMa3tnEg',
        'gallery': [
            {
                'id': 'pintor01',
                'url': '/images/pintor01.jpg',
                'description': 'A través de un espectáculo de títeres de mesa, llevamos a escena “El Pintor de Mariposas” de Rafael Santos Torroella, gracias al apoyo del Programa Creadores Escénicos 2013.'
            },
            {
                'id': 'pintor02',
                'url': '/images/pintor02.jpg',
                'description': ''
            },
            {
                'id': 'pintor_01',
                'url': '/images/pintor_thumb_01.jpg'
            },
            {
                'id': 'pintor_02',
                'url': '/images/pintor_thumb_02.jpg'
            },
            {
                'id': 'pintor_03',
                'url': '/images/pintor_thumb_03.jpg'
            },
            {
                'id': 'pintor_04',
                'url': '/images/pintor_thumb_04.jpg'
            }
        ]
    },
    {
        'id': 'Circo',
        'name': "El Circo de la Muerte", 
        'intro':'Desde las gradas del circo, con su risa encantadora, apareció brinco y brinco la Muerte de anunciadora...',
        'more': 'Entre magos y sombreros, risas y malabares, la muerte con gran esmero sorprendió a sus visitantes. Para que nadie se pierda este circo apabullante, la anfitriona nos promete un reparto impresionante. Todos tomen sus asientos y que nadie se distraiga, que si los cacha la muerte a la tumba se los carga.',
        'credits': [
            {
                'rol': 'Dirección e Idea Original',
                'name': 'Alfredo Payán'
            },
            {
                'rol': 'Producción',
                'name': 'Sara Don'
            },
            {
                'rol': 'Titiriteros',
                'name': 'Iván Romo, Sara Don/Nidia Becerra, Oscar Albores'
            },
            {
                'rol': 'Elaboración de Títeres y Elementos',
                'name': 'Alfredo Payán, Iván Romo, Juan Carlos Andino y Sara Don'
            },
            {
                'rol': 'Vestuario',
                'name': 'Sara Don'
            },
            {
                'rol': 'Diseño',
                'name': 'Alfredo Payán'
            },
            {
                'rol': 'Fotografía',
                'name': 'Tristán Guercovich'
            },
            {
                'rol': 'Edición de Sonido',
                'name': 'Alfredo Payán'
            },
            {
                'rol': 'Música',
                'name': 'Noé Castro'
            },
            {
                'rol': '“El Corrido de la Muerte”',
                'name': 'Alfredo Payán, Iván Romo y Sara Don'
            }
        ],
        'mainImg':'/images/circo_1.jpg',
        'imgPos': 'up',
        'gallery': [
            {
                'id': 'circo_1',
                'url': '/images/circo01.jpg',
                'description': 'El circo de la muerte es un espectáculo de títeres y teatro negro.'
            },
            {
                'id': 'circo_2',
                'url': '/images/circo_2.jpg',
                'description': 'Valiéndose del legendario arte del circo y tomando elementos representativos del folclor mexicano, se lleva a escena un trabajo de ilusiones visuales, donde la Muerte, acompañada por un gusano, recrea a los personajes clásicos del circo: desde el payaso, un fallido mago, el vendedor de golosinas, el zanquero, un despistado domador...'
            },
            {
                'id': 'circo_3',
                'url': '/images/circo_3.jpg',
                'description': 'De esta manera Maika Teatro rinde homenaje a una de las tradiciones más mexicanas: El día de muertos.'
            },
            {
                'id': 'circo01',
                'url': '/images/circo_thumb_1.jpg',
            },
            {
                'id': 'circo02',
                'url': '/images/circo_thumb_2.jpg',
            }
        ]
    },
    {
        'id': 'Principe',
        'name': "El Principe Feliz", 
        'intro':'En lo alto de una columna se alzaba la estatua de oro del Príncipe Feliz. Admirado por todos los habitantes de la ciudad que protege, se siente triste. ¿Cuál será el secreto de sus lágrimas?...', 
        'more':'“Cuando la magia de un cuento nos acompaña desde la niñez, pasa de ser la creación individual de un autor, a un legado universal”.',
        'credits': [
            {
                'rol': 'Dirección Adaptación y Animación',
                'name': 'Alfredo Payán'
            },
            {
                'rol': 'Producción',
                'name': 'Sara Don'
            },
            {
                'rol': 'Elaboración de Títeres',
                'name': 'Alfredo Payán, Juan Carlos Andino'
            },
            {
                'rol': 'Escenografía',
                'name': 'Iván Romo y Alfredo Payán'
            },
            {
                'rol': 'Música original',
                'name': 'Noé Castro y Said Ramos'
            }       
        ],
        'mainImg':'/images/principe_1.jpg',
        'imgPos': 'down',
        'video': 'https://www.youtube.com/embed/rwNevRlBcpM',
        'gallery': [
            {
                'id': 'principe01',
                'url': '/images/principe_01.jpg',
                'description': 'En lo alto de una columna se alzaba la estatua de oro del Príncipe Feliz. Admirado por todos los habitantes de la ciudad que protege, se siente triste. ¿Cuál será el secreto de sus lágrimas?...'
            },
            {
                'id': 'principe02',
                'url': '/images/principe_02.jpg',
                'description': 'El Príncipe Feliz narra la tierna historia de amor entre una estatua y una golondrina, dos seres compasivos ante las atrocidades de la humanidad y que a través de la solidaridad y la entrega descubren que gracias al afecto que los une son capaces de superarlo todo, incluso la muerte.'
            },
            {
                'id': 'principe_01',
                'url': '/images/principe_thumb_1.jpg',
            },
            {
                'id': 'principe_02',
                'url': '/images/principe_thumb_2.jpg',
            },
            {
                'id': 'principe_03',
                'url': '/images/principe_thumb_3.jpg',
            },
            {
                'id': 'principe_04',
                'url': '/images/principe_thumb_4.jpg',
            },
            {
                'id': 'principe_05',
                'url': '/images/principe_thumb_5.jpg',
            },
            {
                'id': 'principe_06',
                'url': '/images/principe_thumb_6.jpg',
            }
        ]
    },
    {
        'id': 'Puente',
        'name': "El puente que construyó el diablo", 
        'intro':'Cada verano es la misma historia en el pueblo de Navacoyán, la creciente del río los deja incomunicados con el resto del mundo; cansado de esta situación, el cacique del pueblo decide contratar al mejor albañil de la región para que construya un puente capaz de poner fin al problema...', 
        'more': '',
        'credits': [
            {
                'rol': 'Adaptación, Dirección y Elaboración de Títeres',
                'name': 'Ana Castillo, Jaime Contreras y Alfredo Payán'
            }         
        ],
        'mainImg':'/images/puente_1.jpg',
        'imgPos': 'up',
        'gallery': [
            {
                'id': 'puente01',
                'url': '/images/puente01.jpg',
                'description': 'Sin embargo, incapaz de concluir la obra decide establecer un pacto con el Diablo, quien se compromete a terminar el puente en una sola noche.'
            },
            {
                'id': 'puente02',
                'url': '/images/puente02.jpg',
                'description': 'Y es una de tantas veces que el diablo siendo tan diablo, no terminó totalmente la obra y perdió el trato que había concertado.'
            },
            {
                'id': 'puente_1',
                'url': '/images/puente_thumb_1.jpg',
            },
            {
                'id': 'puente_2',
                'url': '/images/puente_thumb_2.jpg',
            },
            {
                'id': 'puente_3',
                'url': '/images/puente_thumb_3.jpg',
            }
        ]
    }
]